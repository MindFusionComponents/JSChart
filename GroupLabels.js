define(["require", "exports", 'Scripts/MindFusion.Charting'], function (require, exports, m) {
    ...
    for (var i = 0; i < 4; i++) {
        if (i == 0) {
            series.add(new Charting.BarSeries(new Collections.List([20, 60, 40, 55]), new Collections.List(["Acer", "Biostar", "Foxconn", "Supermicro"]), new Collections.List(["", "", "", ""])));
        }
        if (i == 1) {
            series.add(new Charting.BarSeries(new Collections.List([30, 70, 65, 19]), new Collections.List(["Biostar", "Intel", "Nvidia", "VIA Technologies"]), new Collections.List(["", "", "", ""])));
        }
        if (i == 2) {
            series.add(new Charting.BarSeries(new Collections.List([22, 44, 33, 66]), new Collections.List(["Foxconn", "Nvidia", "Marvell", "NexGen"]), new Collections.List(["", "", "", ""])));
        }
        if (i == 3) {
            series.add(new Charting.BarSeries(new Collections.List([12, 45, 77, 90]), new Collections.List(["Supermicro", "VIA Technologies", "NexGen", "Toshiba"]), new Collections.List(["", "", "", ""])));
        }
    }
    // create labels data
    var annotations = new Collections.ObservableCollection();
    annotations.add(new Charting.BarSeries(new Collections.List([0, 0, 0, 0]), new Collections.List(["", "", "", ""]), new Collections.List(["", "", "", ""]), new Collections.List(["Motherboards", "Chipsets", "CPU", "HDD"])));
...
//# sourceMappingURL=GroupLabels.js.map