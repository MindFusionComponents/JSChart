...
export class StockPriceWithTooltipSeries
	extends MindFusion.Charting.StockPriceSeries
{
	public constructor(values: MindFusion.Charting.Collections.List<MindFusion.Charting.StockPrice> | Array<MindFusion.Charting.StockPrice>)
	{
		super(values);
	}
...
// create sample data
let dataList = new Collections.List<MindFusion.Charting.StockPrice>();
for (let i = 0; i < 30; i++)
{
...
var data = new Collections.ObservableCollection<MindFusion.Charting.Series>();
data.add(series)
...
chart.plot.seriesStyle = new Charting.CandlestickSeriesStyle(
	...
	2, Drawing.DashStyle.Solid, <MindFusion.Charting.CandlestickRenderer>chart.plot.seriesRenderers.item(0));
...
{
	(chart.series.item(0) as MindFusion.Charting.StockPriceSeries).dateTimeFormat = cbDateFormat.selectedIndex;
	chart.draw();
};
...
{
	(chart.series.item(0) as MindFusion.Charting.StockPriceSeries).customDateTimeFormat = tbxCustomDateFormat.value;
};
